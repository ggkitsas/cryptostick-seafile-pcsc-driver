                ...
                size_t datalen, u8 ** out, size_t * outlen)                                                                                                                                                                       
{
    ...
    if (tag_len > 1)   {
        if ((tag_char[tag_len - 1] & SC_ASN1_TAG_PRIMITIVE) != SC_ASN1_TAG_ESCAPE_MARKER)
            SC_TEST_RET(SC_ERROR_INVALID_DATA, "First byte of the long tag is not 'escape marker'");                                                                                                           

        for (ii = 1; ii < tag_len - 1; ii++)
            if (!(tag_char[ii] & 0x80))        
                SC_TEST_RET(SC_ERROR_INVALID_DATA, "MS bit expected to be 'one'");                                                                                                                             

        if (tag_char[0] & 0x80)            
            SC_TEST_RET( SC_ERROR_INVALID_DATA, "MS bit of the last byte expected to be 'zero'");                                                                                                               
    }  
    ...
    *outlen = tag_len + 1 + c + datalen; 
    buf = (unsigned char*)malloc(*outlen);
    if (buf == NULL)
        SC_FUNC_RETURN(SC_ERROR_OUT_OF_MEMORY);                                                                                                                                                                
