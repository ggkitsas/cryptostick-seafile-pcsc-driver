...
/*
static int
pgp_put_data(card_t *card, unsigned int tag, const u8 *buf, size_t buf_len);

static int
pgp_get_data(card_t *card, unsigned int tag, u8 *buf, size_t buf_len);
...
static struct do_info       pgp2_objects[] = {/
    ...
    // The 7F21 is constructed DO in spec, buta in practice, its content can be retrieved as simple DO (no need to parse TLV).
    ...
    { 0xa401, (_type)SIMPLE,      (_access)(READ_ALWAYS | WRITE_PIN3), pgp_get_pubkey_pem, NULL        },
    { 0xb600, (_type)CONSTRUCTED, (_access)(READ_ALWAYS | WRITE_NEVER),  pgp_get_pubkey,     NULL        },
    { 0xb601, (_type)SIMPLE,      (_access)(READ_ALWAYS | WRITE_PIN3), pgp_get_pubkey_pem, NULL        },
    { 0xb800, (_type)CONSTRUCTED, (_access)(READ_ALWAYS | WRITE_NEVER),  pgp_get_pubkey,     NULL        },
    { 0xb801, (_type)SIMPLE,      (_access)(READ_ALWAYS | WRITE_PIN3), pgp_get_pubkey_pem, NULL        },                                                                                                                                                
    { 0, 0, 0, NULL, NULL },
};
...
static int pgp_read_blob(card_t *card, struct blob *blob)
{
    if (blob->data != NULL)
        return SC_SUCCESS;
    if (blob->info == NULL)
...
static int pgp_enumerate_blob(card_t *card, struct blob *blob)
{
    const u8    *in;
    int     r;
...
/*        if ((new = pgp_new_blob(card, blob, tag, sc_file_new())) == NULL)
            return SC_ERROR_OUT_OF_MEMORY;
        pgp_set_blob((blob*)new, data, len);
        in = data + len;
    }
...
/*
static int pgp_get_blob(card_t *card, struct blob *blob, unsigned int id, struct blob **ret)
{
    struct blob *child;
    int r;
...
/*
static struct blob *
pgp_find_blob(card_t *card, unsigned int tag)
{
    struct pgp_priv_data *priv = DRVDATA(card);
    struct blob *blob = NULL;
    int r;
...
int _sc_card_add_algorithm(card_t *card, const sc_algorithm_info_t *info)
{
...
int _sc_card_add_rsa_alg(card_t *card, unsigned int key_length,
...
/*
static int
pgp_get_pubkey_pem(card_t *card, unsigned int tag, u8 *buf, size_t buf_len)                                                                                                                   
{
    struct pgp_priv_data *priv = DRVDATA(card);
    ...
    if ((r = pgp_get_blob(card, priv->mf, tag & 0xFFFE, &blob)) < 0
     ...
     || (r = pgp_read_blob(card, exp_blob)) < 0)
        LOG_TEST_RET( r, "error getting elements");
    ...
    LOG_TEST_RET( r, "public key encoding failed");
